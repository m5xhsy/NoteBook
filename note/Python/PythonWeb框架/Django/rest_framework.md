...
!> 注意：使用时须将`"rest_framework"`注册到项目settings中的app配置中。
...
(12) 如果接收到的是POST请求，执行request.data
    
(13) 所有的解析工作都在request.data中实现，且data是一个方法(被装饰后的)
...
(15) 执行self._load_data_and_files()
    
(16) 执行self._data, self._file = self.parse()
    
(17) parser = self.negotiator.select_parser(self, self.parsers)
    
(18) 点进self.parsers发现self.parsers = parsers or ()是request实例化过来的
    
(19) 进dispatch()查看实例化reques的部分并点进request = self.initialize_request(request, *args, **kwargs)
    
(20) get_parsers(self)返回的为[parser() for parser in self.parser_classes]，而self表示的是调用dispatch的类，也就是视图类
    
(21) 如果视图类设置了parser_classes则第11.7中的self为设置的解析器，否则在APIView中找
    
(22) 在APIView中parser_classes = api_settings.DEFAULT_PARSER_CLASSES
    
(23) 点击进入api_settings发现api_settings是APISettings(None, DEFAULTS, IMPORT_STRINGS)的实例，进去找DEFAULT_PARSER_CLASSES属性，没有这个属性执行__getattr__(self, attr)
    
(24) self.defaults是实例化传进来的，attr在self.defaults里面
    
(25) 执行self.user_settings,由于实例化时user_settings传进来为None，所以self._user_settings = getattr(settings, 'REST_FRAMEWORK', {})去settings中找'REST_FRAMEWORK'，如果没找到取空字典，并返回
    
(26) 返回的self.user_settings如果是空字典，则self.user_settings[attr]会报一个KeyError的错误并被捕获

(27) 捕获后在self.defaults[attr]找到并return出去

(28) 在LoginView里面找到对应的方法，执行该方法，最后返回给用户
```
...
!>浏览器访问http://127.0.0.1:8000/ass/mixinboks.json可返回json数据；路由进行其他操作见rest_framework官网https://www.django-rest-framework.org/
